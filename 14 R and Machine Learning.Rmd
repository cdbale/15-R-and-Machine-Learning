...
"Please know this: if everything and everyone else in the world whom you should trust should fail, Jesus Christ and His Church will *never* fail you. The Lord never slumbers, nor does He sleep. He 'is the same yesterday, today, and [tomorrow].' He will not forsake His covenants, His promises, or His love for His people. He works miracles today, and He will work miracles tomorrow."
...
While we can use inferential models to predict, predictive models are (often) black boxes that do not estimate coefficients that give us information about our story.
...
Write a for-loop that fills in a vector of 10 values. Place a zero at every even index, and place the value of every odd index at its corresponding position in the vector.

Hint: use the modulo `%%` to determine when an index is even. The modulo returns the remainder from dividing the number on its left by the number on its right.

## Desired Output

```{r echo = FALSE}
empty_vector <- vector(mode = "double", length = 10)
...
1. Write a function for simulating data that uses conditional statements and a for loop. Call it and print the output.
